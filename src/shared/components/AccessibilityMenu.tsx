...
export function AccessibilityMenu() {
  ...
  useEffect(() => {
    ...
    // Priority Mode
    if (priorityMode === 'child') {
      root.classList.add('priority-child');
      root.classList.remove('priority-disability');
    } else if (priorityMode === 'disability') {
      root.classList.add('priority-disability');
      root.classList.remove('priority-child');
    } else {
      root.classList.remove('priority-child', 'priority-disability');
    }
    localStorage.setItem('pref_priority_mode', priorityMode);
  }, [fontSize, highContrast, ttsEnabled, priorityMode]);
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="bg-white border-4 border-[#141414] shadow-[8px_8px_0px_0px_rgba(20,20,20,1)] p-6 rounded-3xl w-80 mb-4 space-y-6 overflow-y-auto max-h-[80vh]"
          >
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-2 text-[10px] font-bold uppercase opacity-40">
                  <Accessibility size={14} /> Priority Mode
                </div>
                <div className="grid grid-cols-3 gap-2">
                  {['standard', 'child', 'disability'].map(mode => (
                    <button
                      key={mode}
                      onClick={() => setPriorityMode(mode)}
                      className={`py-2 text-[10px] font-black uppercase border-2 rounded-xl transition-all ${
                        priorityMode === mode ? 'bg-indigo-600 text-white border-indigo-600' : 'border-gray-100 hover:border-[#141414]'
                      }`}
                    >
                      {mode}
                    </button>
